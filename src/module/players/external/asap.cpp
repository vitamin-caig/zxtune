 ...
 * @file
 *
 * @brief  ASAP-based formats support implementation
 *
...
{
  const Debug::Stream Dbg("Module::ASAP");
  ...
  {
  public:
    explicit MultitrackFactory(StringView id)
      : Id(id)
    {}
                             ...
                             Parameters::Container::Ptr properties) const override
    {
      try
      {
        auto tune = MakePtr<AsapTune>(Id, container, container.StartTrackIndex());
      ...
      {
        Dbg("Failed to create {}: {}", Id, e.ToString());
      }
      return {};
    }
  ...
  public:
                             ...
                             Parameters::Container::Ptr properties) const override
    {
      try
      {
        auto tune = MakePtr<AsapTune>(Id, container, 0);
  ...
  {
    return MakePtr<MultitrackFactory>(id);
  }
