...
namespace ZXTune::VGMStream
{
  using Module::VGMStream::PluginType;
...
namespace ZXTune
{
  void RegisterVGMStreamPlugins(PlayerPluginsRegistrator& players, ArchivePluginsRegistrator& archives)
  {
    ...
    for (const auto& desc : VGMStream::PLUGINS)
    {
      Require(formats.insert(desc.Format).second);
      if (desc.Type == VGMStream::PluginType::MULTITRACK)
      {
        auto decoder = Formats::Multitrack::CreateVGMStreamDecoder(desc.Format, desc.Description, desc.Suffix);
        auto factory = Module::VGMStream::CreateMultitrackFactory(desc.Id, desc.Description, desc.Suffix);
        {
      ...
      {
        const auto trait = desc.Type == VGMStream::PluginType::MULTIFILE ? Capabilities::Module::Traits::MULTIFILE : 0;
        auto decoder = Formats::Chiptune::CreateFormatDecoder(desc.Format, desc.Description);
        auto factory = Module::VGMStream::CreateFactory(desc.Id, desc.Description, desc.Suffix, desc.Type);
