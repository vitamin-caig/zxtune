...
namespace ProTracker1
{
  ...
  private:
    ...
    void SynthesizeChannel(ChannelState& dst, AYM::ChannelBuilder& channel, AYM::TrackBuilder& track)
    {
      ...
      //apply tone
      const int_t halftones = clamp<int_t>(int_t(dst.Note) + curOrnament.GetLine(dst.PosInSample), 0, 95);
...
namespace ZXTune
{
  void RegisterPT1Support(PluginsRegistrator& registrator)
  {
    //direct modules
    {
      ...
      const PlayerPlugin::Ptr plugin = CreatePlayerPlugin(PT1::ID, decoder->GetDescription() + Text::PLAYER_DESCRIPTION_SUFFIX, PT1::CAPS, factory);
