...
namespace Formats::Packed
{
  namespace TurboLZ
  {
    ...
    struct Simple
    {
      ...
      struct KeyFunc
      {
        explicit KeyFunc(const Binary::View /*data*/) {}
    ...
    struct Protected
    {
      ...
      struct KeyFunc
      {
        explicit KeyFunc(Binary::View data)
          : Index(Key[offsetof(RawHeader, InitialCryptoKeyIndex) - KeyOffset])
        {
          const auto src = data.SubView(KeyOffset, Key.size());
          Require(src.Size() == Key.size());
          std::copy_n(src.As<uint8_t>(), Key.size(), Key.begin());
        }
    ...
    public:
      ...
      auto GetPackedData() const
      {
        ...
        return Data.SubView(header.FIXED_PART_SIZE + header.DepackerBodySize, header.PackedDataSize);
      }
      ...
      auto GetKeyFunc() const
      {
        return typename Version::KeyFunc(Data);
      }
    ...
    public:
        ...
        , Stream(parser.GetPackedData())
      {
        ...
        {
          auto keyFunctor = parser.GetKeyFunc();
  ...
  {
    return MakePtr<TurboLZ::Decoder<TurboLZ::Simple>>();
  }

  Decoder::Ptr CreateTurboLZProtectedDecoder()
  {
    return MakePtr<TurboLZ::Decoder<TurboLZ::Protected>>();
  }
}  // namespace Formats::Packed
