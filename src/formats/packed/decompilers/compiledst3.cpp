  ...
  namespace CompiledST3
  {
    ...
    public:
      ...
      Container::Ptr Decode(const Binary::Container& rawData) const override
      {
        ...
        auto& stub = Chiptune::SoundTracker::GetStubBuilder();
        if (IsInfoEmpty(metainfo))
        {
          Dbg("Player has empty metainfo");
          if (auto originalModule = Chiptune::SoundTracker::Ver3::Parse(*modData, stub))
          {
            const std::size_t originalSize = originalModule->Size();
            return CreateContainer(std::move(originalModule), playerSize + originalSize);
          }
        }
        else if (auto fixedModule = Chiptune::SoundTracker::Ver3::InsertMetainformation(*modData, metainfo))
        {
          if (Chiptune::SoundTracker::Ver3::Parse(*fixedModule, stub))
          {
    ...
    private:
      const Binary::Format::Ptr Format = Binary::CreateFormat(FORMAT, sizeof(RawPlayer));
    };
  }  // namespace CompiledST3

  Decoder::Ptr CreateCompiledST3Decoder()
  {
    return MakePtr<CompiledST3::Decoder>();
  }
}  // namespace Formats::Packed
