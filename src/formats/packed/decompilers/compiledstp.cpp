  ...
  namespace CompiledSTP
  {
    ...
    public:
      ...
      Container::Ptr Decode(const Binary::Container& rawData) const override
      {
        ...
        auto& stub = Chiptune::SoundTrackerPro::GetStubBuilder();
        if (IsInfoEmpty(metainfo))
        {
          Dbg("Player has empty metainfo");
          if (auto originalModule = Chiptune::SoundTrackerPro::ParseCompiled(*modData, stub))
          {
            const std::size_t originalSize = originalModule->Size();
            return CreateContainer(std::move(originalModule), playerSize + originalSize);
          }
        }
        else if (auto fixedModule = Chiptune::SoundTrackerPro::InsertMetaInformation(*modData, metainfo))
        {
          if (Chiptune::SoundTrackerPro::ParseCompiled(*fixedModule, stub))
          {
    ...
    private:
      const Binary::Format::Ptr Format = Binary::CreateFormat(Version::FORMAT, sizeof(typename Version::RawPlayer));
    };
  }  // namespace CompiledSTP

  Decoder::Ptr CreateCompiledSTP1Decoder()
  {
    return MakePtr<CompiledSTP::Decoder<CompiledSTP::Version1> >();
  }

  Decoder::Ptr CreateCompiledSTP2Decoder()
  {
    return MakePtr<CompiledSTP::Decoder<CompiledSTP::Version2> >();
  }
}  // namespace Formats::Packed
