...
namespace Formats::Packed
{
  namespace CompiledPTU13
  {
    ...
    namespace ProTracker3 = Chiptune::ProTracker3;
    ...
    public:
      ...
      Container::Ptr Decode(const Binary::Container& rawData) const override
      {
        ...
        if (positionsAddr < playerSize + offsetof(RawHeader, Positions))
        {
          Dbg("Invalid compile addr");
          return {};
        }
        const uint_t dataAddr = positionsAddr - offsetof(RawHeader, Positions);
        const auto modData = data.SubView(playerSize, MAX_MODULE_SIZE);
        ...
        const auto builder = Chiptune::PatchedDataBuilder::Create(modData);
        // fix patterns/samples/ornaments offsets
        for (uint_t idx = offsetof(RawHeader, PatternsOffset); idx != offsetof(RawHeader, Positions); idx += 2)
        {
    ...
    private:
      const Binary::Format::Ptr Format = Binary::CreateFormat(FORMAT, PLAYER_SIZE + sizeof(RawHeader));
    };
  }  // namespace CompiledPTU13

  Decoder::Ptr CreateCompiledPTU13Decoder()
  {
    return MakePtr<CompiledPTU13::Decoder>();
  }
}  // namespace Formats::Packed
