...
#include "formats/packed/images/utils.h"
...
namespace Formats::Packed
{
  namespace TeleDiskImage
  {
    ...
    public:
      ...
      virtual void OnSector(const CHS& loc, Binary::Data::Ptr data, SectorDataType type, std::size_t targetSize) = 0;
    };
    ...
    auto DecodeR2P(Binary::View data)
    {
      ...
      for (const auto *it = data.As<R2PEntry>(), *lim = it + data.Size() / sizeof(*it); it != lim; ++it)
      {
    ...
    {
    public:
      explicit ImageVisitorAdapter(ImageBuilder& builder)
        : Builder(builder)
      {}

      void OnSector(const CHS& loc, Binary::Data::Ptr data, SectorDataType type, std::size_t targetSize) override
      {
        switch (type)
        {
        case RAW_SECTOR:
    ...
    void ParseSectors(Binary::InputStream& stream, bool hasComment, ImageVisitor& visitor)
    {
      ...
      for (;;)
      {
        ...
        for (uint_t sect = 0; sect != track.Sectors; ++sect)
        {
          ...
          const CHS loc(sector.Cylinder, track.Head, sector.Number);
    ...
    public:
      ...
      Container::Ptr Decode(const Binary::Container& rawData) const override
      {
        ...
        ImageVisitorAdapter visitor(*builder);
        if (const std::size_t usedSize = Parse(rawData, visitor))
        {
  ...
  {
    return MakePtr<TeleDiskImage::Decoder>();
  }
}  // namespace Formats::Packed
