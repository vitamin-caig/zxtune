...
namespace Formats::Packed
{
  namespace DSK
  {
    ...
    {
    public:
      explicit Parser(ImageBuilder& target)
        : Target(target)
      {}
    ...
    private:
      void ParseTrack(Binary::View trackData)
      {
        ...
        for (uint_t sector = 0; sector != trackInfo.SectorsCount; ++sector)
        {
          ...
          Target.SetSector(CHS(sectorInfo.Track, sectorInfo.Side, sectorInfo.Sector), sectorData);
        }
      }
      ...
      void ParseExtendedTrack(Binary::View trackData)
      {
        ...
        for (uint_t sector = 0; sector != trackInfo.SectorsCount; ++sector)
        {
          ...
          if (const std::size_t sectorSize = sectorInfo.ActualDataSize)
          {
            ...
            Target.SetSector(CHS(sectorInfo.Track, sectorInfo.Side, sectorInfo.Sector),
    ...
    public:
      ...
      Container::Ptr Decode(const Binary::Container& rawData) const override
      {
        ...
        try
        {
          ...
          const std::size_t usedSize = Parser(*builder).Parse(rawData);
