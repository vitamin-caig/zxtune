...
namespace
{
  const std::string THIS_MODULE("Formats::Chiptune::ProSoundMakerCompiled");
}
    ...
    public:
      ...
      void ParseOrnaments(const Indices& ornaments, Builder& builder) const
      {
        ...
        if (Source.OrnamentsOffset == Source.PatternsOffset)
        {
          ...
          Log::Debug(THIS_MODULE, "No ornaments. Making stub");
  ...
  namespace ProSoundMaker
  {
    Formats::Chiptune::Container::Ptr ParseCompiled(const Binary::Container& rawData, Builder& target)
    {
      ...
      try
      {
        ...
        return boost::make_shared<ProSoundMakerCompiled::Container>(rawData, format.GetSize(), format.GetFixedArea());
      }
