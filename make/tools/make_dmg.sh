...
Available options:
  ...
  -n <name>           Specify resulting dmg path. If not specified, defaults to BundleName.dmg.
...
while getopts ":d:i:b:c:s:n:t:Vvhl" opt; do
	...
	s)
		...
		;;
	n)
		echo "Setting dmg path to $OPTARG"
		ARG_DMG_PATH=$OPTARG
		;;
	N)
...
DMG_PATH="${ARG_DMG_PATH}"
if [ ! "${DMG_PATH}" ]; then
	DMG_PATH=$(dirname ${APP_BUNDLE_PATH})/${APP_BUNDLE_NAME}.dmg
	echo "Defaulting dmg path to ${DMG_PATH}"
fi
DMG_PATH_TMP="${DMG_PATH}.tmp.dmg"
...
rm -f "${DMG_PATH_TMP}"
hdiutil create -ov -srcfolder $TMP_DIR -format UDRW -volname "${VOL_NAME}" "${DMG_PATH_TMP}"
