// Generated from '$Id: $'
#ifndef __TEXT_BACKENDS_H_DEFINED__
#define __TEXT_BACKENDS_H_DEFINED__
#include <char_type.h>

extern const Char ALSA_BACKEND_KEY[] = {
  'a','l','s','a',0
};
extern const Char FILE_ANNOTATION_EXT[] = {
  '.','i','n','f','o',0
};
extern const Char FILE_ANNOTATION_FORMAT[] = {
  '%','1','%','=','%','2','%','\n',
  0
};
extern const Char FILE_BACKEND_KEY[] = {
  'f','i','l','e',0
};
extern const Char FILE_WAVE_EXT[] = {
  '.','w','a','v',0
};
extern const Char NULL_BACKEND_KEY[] = {
  'n','u','l','l',0
};
extern const Char OSS_BACKEND_KEY[] = {
  'o','s','s',0
};
extern const Char TEXT_ALSA_BACKEND_DESCRIPTION[] = {
  'A','L','S','A',' ','s','o','u','n','d',' ','s','y','s','t','e','m',' ','b','a','c','k','e','n','d',0
};
extern const Char TEXT_FILE_BACKEND_DESCRIPTION[] = {
  'F','i','l','e',' ','o','u','t','p','u','t',' ','b','a','c','k','e','n','d',0
};
extern const Char TEXT_NULL_BACKEND_DESCRIPTION[] = {
  'N','u','l','l',' ','o','u','t','p','u','t',' ','b','a','c','k','e','n','d',0
};
extern const Char TEXT_OSS_BACKEND_DESCRIPTION[] = {
  'O','S','S',' ','s','o','u','n','d',' ','s','y','s','t','e','m',' ','b','a','c','k','e','n','d',0
};
extern const Char TEXT_WIN32_BACKEND_DESCRIPTION[] = {
  'W','i','n','3','2',' ','s','o','u','n','d',' ','s','y','s','t','e','m',' ','b','a','c','k','e','n','d',0
};
extern const Char WIN32_BACKEND_KEY[] = {
  'w','i','n','3','2',0
};
#endif //__TEXT_BACKENDS_H_DEFINED__
