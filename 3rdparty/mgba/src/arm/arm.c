...
mLOG_DEFINE_CATEGORY(ARM_CPU, "ARM CPU");
...
static inline void ARMStep(struct ARMCore* cpu) {
...
#ifdef USE_DISASSEMBLER
    {
        ...
        uint32_t vaddr = cpu->gprs[ARM_PC] - WORD_SIZE_ARM;
        if (darm_armv7_disasm(&d, opcode) < 0 ||
            darm_str2(&d, &str, 1) < 0) {
            mLOG(ARM_CPU, DEBUG, "#%06x %08x (..)", vaddr, opcode);
        }
        else {
            mLOG(ARM_CPU, DEBUG, "#%06x %08x %s", vaddr, opcode, str.total);
        }
    }
#endif
...
static inline void ThumbStep(struct ARMCore* cpu) {
    ...
    {
        darm_t d; darm_str_t str;
        uint32_t vaddr = cpu->gprs[ARM_PC] - WORD_SIZE_THUMB;
        if (darm_thumb_disasm(&d, opcode) < 0 ||
            darm_str2(&d, &str, 1) < 0) {
            mLOG(ARM_CPU, DEBUG, "#%06x %04x (..)", vaddr, opcode);
        }
        else {
            mLOG(ARM_CPU, DEBUG, "#%06x %04x %s", vaddr, opcode, str.total);
        }
    }
#endif
	cpu->prefetch[0] = cpu->prefetch[1];
