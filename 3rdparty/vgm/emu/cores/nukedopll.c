...
#include "../EmuHelper.h"
#include "nukedopll.h"
#include "nukedopll_int.h"


static void nukedopll_write(void *chip, UINT8 a, UINT8 v);
...
static void OPLL_GenerateResampled(opll_t *chip, int32_t *buf)
{
    uint32_t i;
    int32_t buffer[2];
    ...
    while (chip->samplecnt >= chip->rateratio)
    {
        ...
        for (i = 0; i < 18; i++)
        {
            ...
            while (chip->writebuf[chip->writebuf_cur].time <= chip->writebuf_samplecnt)
            {
                ...
                NOPLL_Write(chip, chip->writebuf[chip->writebuf_cur].port,
...
static void nukedopll_write(void *chip, UINT8 a, UINT8 v)
{
	NOPLL_WriteBuffered((opll_t *)chip, a, v);
}
...
static void nukedopll_set_mute_mask(void *chipptr, UINT32 MuteMask)
{
	...
	for (chn = 0; chn < 14; chn ++)
		chip->mute[MUTE_MASK_MAP[chn]] = (MuteMask >> chn) & 0x01;
