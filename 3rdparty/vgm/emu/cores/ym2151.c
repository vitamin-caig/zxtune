...
/*
** File: ym2151.c - software implementation of YM2151 FM Operator Type-M(OPM)
**
...
typedef struct
{
	...
	*/
	...
	UINT32      sampfreq;               /* sampling frequency in Hz */
	UINT8       lastreg;
...
static void init_tables(void)
{
	...
	{
		// note: this formula is broken in MAME 0.183
...
static void init_chip_tables(YM2151 *chip)
{
	...
	mult = (1<<FREQ_SH) * scaler / (double)(1<<20);
	for (j=0; j<4; j++)
	{
		for (i=0; i<32; i++)
		{
			/*calculate phase increment*/
			phaseinc = ( dt1_tab[j*32+i] * SIN_LEN);
	...
	mult = (1<<TIMER_SH) * (double)chip->sampfreq / chip->clock;
	for (i=0; i<1024; i++)
	{
		/* ASG 980324: changed to compute both tim_A_tab and timer_A_time */
		chip->tim_A_tab[i] = (64 * (1024 - i)) * mult;  /* number of samples that timer period takes (fixed point) */
	}
	for (i=0; i<256; i++)
	{
		/* ASG 980324: changed to compute both tim_B_tab and timer_B_time */
		chip->tim_B_tab[i] = (1024 * (256 - i)) * mult; /* number of samples that timer period takes (fixed point) */
	}
...
{
	if (v&0x08) /* M1 */
		KEY_ON (op+0, 1, PSG->eg_cnt);
	else
		KEY_OFF(op+0, 1);

	if (v&0x20) /* M2 */
		KEY_ON (op+1, 1, PSG->eg_cnt);
	else
		KEY_OFF(op+1, 1);

	if (v&0x10) /* C1 */
		KEY_ON (op+2, 1, PSG->eg_cnt);
	else
		KEY_OFF(op+2, 1);

	if (v&0x40) /* C2 */
		KEY_ON (op+3, 1, PSG->eg_cnt);
	else
		KEY_OFF(op+3, 1);
}
...
*
*   'chip' is a pointer to the virtual YM2151
...
void ym2151_set_irq_handler(void *chip, void(*handler)(void *param, UINT8 irq))
{
	YM2151 *PSG = (YM2151 *)chip;
...
static void ym2151_w(void *chip, UINT8 offset, UINT8 data)
{
	...
	if (offset & 1)
		ym2151_write_reg(chip, PSG->lastreg, data);
	else
		PSG->lastreg = data;
}
