...
INLINE void do_nor(struct rsp_core* sp, short* VD, short* VS, short* VT)
{
...
static void VNOR(struct rsp_core* sp, int vd, int vs, int vt, int e)
{
    ALIGNED short ST[N];

    SHUFFLE_VECTOR(ST, sp->VR[vt], e);
    do_nor(sp, sp->VR[vd], sp->VR[vs], ST);
    return;
}
