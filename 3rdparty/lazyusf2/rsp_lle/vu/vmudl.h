...
INLINE static void do_mudl(struct rsp_core* sp, short* VD, short* VS, short* VT)
{
...
static void VMUDL(struct rsp_core* sp, int vd, int vs, int vt, int e)
{
    ALIGNED short ST[N];

    SHUFFLE_VECTOR(ST, sp->VR[vt], e);
    do_mudl(sp, sp->VR[vd], sp->VR[vs], ST);
    return;
}
