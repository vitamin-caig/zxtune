...
final class PlaylistFileIterator implements Iterator {
  ...
  public static Iterator create(Context context, Uri path) throws IOException {
    ...
    final PlaylistFileIterator result = new PlaylistFileIterator(getParentOf(path), delegate, handler);
    if (!result.next()) {
  ...
  private PlaylistFileIterator(URI dir, ReferencesIterator delegate, VfsIterator.ErrorHandler handler) {
    this.dir = dir;
