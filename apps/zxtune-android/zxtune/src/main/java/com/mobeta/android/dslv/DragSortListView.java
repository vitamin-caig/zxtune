...
public class DragSortListView extends ListView {
  ...
  /**
   * The float View location. First based on touch location and given deltaX and
   * deltaY. Then restricted by callback to FloatViewManager.onDragFloatView().
  ...
  /**
   * While drag-sorting, the current position of the floating View. If dropped,
   * the dragged item will land in this position.
   */
  ...
  /**
   * A listener that receives callbacks whenever the floating View hovers over a
   * new position.
   */
  ...
  /**
   * Height in pixels to which the originally dragged item is collapsed during a
   * drag-sort. Currently, this value must be greater than zero.
   */
  ...
  /**
   * Defines the scroll speed during a drag-scroll. User can provide their own;
   * this default is a simple linear profile where scroll speed increases
  ...
  /**
   * Drag flag bit. Floating View can move in the positive y direction. This is
   * subtle. What this actually means is that, if enabled, the floating View can
   * be dragged below its starting position. Remove in favor of upper-bounding
   * item position?
   */
  public final static int DRAG_POS_Y = 0x4;

  /**
   * Drag flag bit. Floating View can move in the negative y direction. This is
   * subtle. What this actually means is that the floating View can be dragged
   * above its starting position. Remove in favor of lower-bounding item
   * position?
   */
   ...
   * (only used if drag-sort animations are turned on). Nearly 1 means the item
   * is at the top of the slide region (nearly full blank item is directly
   * below).
   */
   ...
   * get the item from the ListAdapter to obtain its height. That process can be
   * expensive, but often the same offscreen item will be requested many times
   * in a row. Once an offscreen item height is calculated, we cache it in this
   * guy. Actually, we cache the height of the child of the DragSortItemView
   * since the item height changes often during a drag-sort.
   */
   ...
   * <code>adapter</code>, this method calls the appropriate set*Listener method
   * with <code>adapter</code> as the argument.
   * 
   ...
   *          this height.
   * @return Shuffle line between position-1 and position (for the given view of
   *         the list; that is, for when top of item at position has y-coord of
   *         given `top`). If floating View (treated as horizontal line) is
   *         dropped immediately above this line, it lands in position-1. If
  ...
  /**
   * Stop a drag in progress. Pass <code>true</code> if you would like to remove
   * the dragged item from the list.
   * 
   * @param remove Remove the dragged item from the list. Calls a registered
   *          RemoveListener, if one exists. Otherwise, calls the DropListener,
   *          if one exists.
   ...
   *          0.5f.
   * @param lowerFrac Fraction of ListView height for down-scroll bound. Capped
   *          at 0.5f.
   */
   ...
   * FloatViewManager. Calls through to {@link #startDrag(int,View,int,int,int)}
   ...
   *         <code>startDrag</code> will fail if we are not currently in a touch
   *         event, there is no registered FloatViewManager, or the
   ...
   * @return True if the drag was started, false otherwise. This
   *         <code>startDrag</code> will fail if we are not currently in a touch
   *         event, <code>floatView</code> is null, or there is a drag in
   *         progress.
   */
  ...
  public interface FloatViewManager {
     ...
     * DragSortListView will measure and layout this View for you, so feel free
     * to just inflate it. You can help DSLV by setting some
     * {@link ViewGroup.LayoutParams} on this View; otherwise it will set some
     * for you (with a width of FILL_PARENT and a height of WRAP_CONTENT).
     * 
    ...
    /**
     * Called whenever the floating View is dragged. Float View properties can
     * be changed here. Also, the upcoming location of the float View can be
     * altered by setting <code>location.x</code> and <code>location.y</code>.
     * 
     * @param floatView The floating View.
     * @param location The location (top-left; relative to DSLV top-left) at
     *          which the float View would like to appear, given the current
     *          touch location and the offset provided in
  ...
  /**
   * This better reorder your ListAdapter! DragSortListView does not do this for
   * you; doesn't make sense to. Make sure
  ...
  /**
   * Use this to move the check state of an item from one position to another in
   * a drop operation. If you have a choiceMode which is not none, this method
   * must be called when the order of items changes in an underlying adapter
   * which does not have stable IDs (see {@link ListAdapter#hasStableIds()}).
   * This is because without IDs, the ListView has no way of knowing which items
   * have moved where, and cannot update the check state accordingly.
   * <p>
   ...
   * the naive algorithm used, ListView will only search for an ID in the close
   * neighborhood of the old position. If the user moves an item too far
   * (specifically, more than 20 rows away), ListView will give up and just
   * force the item to be unchecked. So if there is a reasonable chance that the
   * user will move items more than 20 rows away from the original position, you
   * may wish to use an adapter with unstable IDs and call this method manually
   * instead.
   * 
   * @param from
   * @param to
   ...
   * this method must be called when the order of items changes in an underlying
  ...
  /**
   * Interface for controlling scroll speed as a function of touch position and
   * time. Use {@link DragSortListView#setDragScrollProfile(DragScrollProfile)}
  ...
  public interface DragScrollProfile {
     ...
     * number.
     * 
     * @param w Normalized position in scroll region (i.e. w \in [0,1]). Small w
     *          typically means slow scrolling.
     * @param t Time (in milliseconds) since start of scroll (handy if you want
     *          scroll acceleration).
