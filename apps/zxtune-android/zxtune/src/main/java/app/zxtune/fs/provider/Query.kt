 ...
 */
internal class Query private constructor(val providerUri: ContentUri, val type: Type?) {
    ...
    companion object {
        ...
        fun parse(uri: Uri) = Query(ContentUri.from(uri), Type.entries.getOrNull(uriTemplate.match(uri)))
        ...
        private fun makeSimple(type: Type, uri: Uri) = makeComplex(type, uri) {}

        private fun makeComplex(type: Type, uri: Uri, patch: Uri.Builder.() -> Unit) =
            Query(ContentUri.from(makeUri(type, uri).apply(patch).build()), type)
    }

    val path: Uri
        get() = when (type) {
            Type.RESOLVE, Type.LISTING, Type.FEED, Type.SEARCH, Type.FILE, Type.NOTIFICATION ->
                providerUri[1]?.toUri() ?: Uri.EMPTY

            else -> throw IllegalArgumentException("Wrong URI: $providerUri")
        }

    val searchQuery
        get() = providerUri.takeIf { type == Type.SEARCH }?.get(QUERY_PARAM)
            ?: throw IllegalArgumentException("Wrong search URI: $providerUri")

    val fileSize
        get() = providerUri.takeIf { type == Type.FILE }?.get(SIZE_PARAM)
