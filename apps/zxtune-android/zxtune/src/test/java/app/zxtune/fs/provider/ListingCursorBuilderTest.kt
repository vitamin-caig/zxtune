...
class ListingCursorBuilderTest {
    ...
    private val dirObject1 =
        Schema.Content.Dir(dir1.uri, dir1.name, dir1.description, dirIconUri, false)
    private val dir2 = TestDir(2)
    private val dirObject2 = Schema.Content.Dir(dir2.uri, dir2.name, dir2.description, null, true)

    private val file1 = TestFile(10, "100")
    private val fileObject1 = Schema.Content.File(
        file1.uri,
        file1.name,
        file1.description,
        file1IconUri,
        file1.size,
        Schema.Content.File.Type.REMOTE
    )
    private val file2 = TestFile(20, "Cached")
    private val fileObject2 = Schema.Content.File(
        file2.uri,
        file2.name,
        file2.description,
        file2IconUri,
        file2.size,
        Schema.Content.File.Type.UNSUPPORTED
    )
    private val file3 = TestFile(30, "Invalid cache")
    private val fileObject3 = Schema.Content.File(
        file3.uri, file3.name, file3.description, null, file3.size, Schema.Content.File.Type.TRACK
    )
    ...
    fun `dirs and files`() {
        ...
        ListingCursorBuilder().apply {
            ...
            moveToNext()
            assertEquals(dirObject1, Schema.Object.parse(this) as Schema.Content.Dir)
            moveToNext()
            assertEquals(dirObject2, Schema.Object.parse(this) as Schema.Content.Dir)
            moveToNext()
            assertEquals(fileObject1, Schema.Object.parse(this) as Schema.Content.File)
            moveToNext()
            assertEquals(fileObject2, Schema.Object.parse(this) as Schema.Content.File)
            moveToNext()
            assertEquals(fileObject3, Schema.Object.parse(this) as Schema.Content.File)
        }
